# Red Hat OpenShift - Exhaustive Interview Preparation Guide

## 1. Mục tiêu học
- Hiểu rõ về Red Hat OpenShift trong môi trường Enterprise.
...
- **Enterprise Context:** Red Hat OpenShift được áp dụng rộng rãi để mở rộng hệ thống.
- **Enterprise Context (Doanh nghiệp Enterprise):** Áp dụng Red Hat OpenShift để đáp ứng JD 60% System + 40% DevOps, tối ưu chi phí và tăng tính ổn định (nhất là hệ thống TOS).
- **Candidate CV Alignment:** Mapping kinh nghiệm AWS, EKS, Docker, K8s, Terraform, ArgoCD vào bối cảnh Red Hat OpenShift.

## 4. Kiến trúc / Cách hoạt động (ASCII Diagrams)
```text
[ Client ] --> [ Load Balancer ] --> [ Red Hat OpenShift Component A ]
                                     |
                                     +--> [ Red Hat OpenShift Component B ]
```
...
- Các tool đặc thù của Red Hat OpenShift.
...
```yaml
# Sample config for Red Hat OpenShift
server:
  port: 8080 
...
## 14. So sánh
| Feature | Red Hat OpenShift | Alternative |
| --- | --- | --- |
...
- **Basic:** Red Hat OpenShift là gì?
- **Intermediate:** Nêu cách backup/restore?
- **Advanced:** Làm sao để scale Red Hat OpenShift cho 1 triệu RPS?
- **Production/Architecture:** Thiết kế Red Hat OpenShift đa vùng (Multi-AZ).
...
- **Q:** Red Hat OpenShift dùng để làm gì? -> **A:** Giải pháp cho ...
