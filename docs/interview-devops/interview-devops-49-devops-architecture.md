# DevOps Architecture - Exhaustive Interview Preparation Guide

## 1. Mục tiêu học
- Hiểu rõ về DevOps Architecture trong môi trường Enterprise.
...
- **Enterprise Context:** DevOps Architecture được áp dụng rộng rãi để mở rộng hệ thống.
- **Enterprise Context (Doanh nghiệp Enterprise):** Áp dụng DevOps Architecture để đáp ứng JD 60% System + 40% DevOps, tối ưu chi phí và tăng tính ổn định (nhất là hệ thống TOS).
- **Candidate CV Alignment:** Mapping kinh nghiệm AWS, EKS, Docker, K8s, Terraform, ArgoCD vào bối cảnh DevOps Architecture.

## 4. Kiến trúc / Cách hoạt động (ASCII Diagrams)
```text
[ Client ] --> [ Load Balancer ] --> [ DevOps Architecture Component A ]
                                     |
                                     +--> [ DevOps Architecture Component B ]
```
...
- Các tool đặc thù của DevOps Architecture.
...
```yaml
# Sample config for DevOps Architecture
server:
  port: 8080 
...
## 14. So sánh
| Feature | DevOps Architecture | Alternative |
| --- | --- | --- |
...
- **Basic:** DevOps Architecture là gì?
- **Intermediate:** Nêu cách backup/restore?
- **Advanced:** Làm sao để scale DevOps Architecture cho 1 triệu RPS?
- **Production/Architecture:** Thiết kế DevOps Architecture đa vùng (Multi-AZ).
...
- **Q:** DevOps Architecture dùng để làm gì? -> **A:** Giải pháp cho ...
