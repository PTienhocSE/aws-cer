# Linux Troubleshooting — Cẩm nang phỏng vấn

# 1. Mục tiêu học
triage CPU/memory/disk/network/process và evidence theo timeline; liên hệ concept với vận hành, failure mode và quyết định production.

# 2. Kiến thức nền cần có
Linux, networking, storage, identity, scripting, observability và change management.

# 3. Tổng quan kiến trúc
Mô tả control plane, data plane, state, dependency, traffic flow và failure domain của Linux Troubleshooting.

# 4. Cách hoạt động
Mô tả lifecycle từ request/config đến execution, persistence, response, audit và metric.

# 5. Thành phần và failure mode
Linux Troubleshooting có thể gặp resource exhaustion, network partition, stale state, permission error, disk failure hoặc bad change; xác định impact của từng lỗi.
...
top, vmstat, iostat, sar, ss, lsof, strace
...
# 16. Knowledge check
Giải thích flow, failure domain, metric quan trọng, cách khoanh vùng và tiêu chí rollback của Linux Troubleshooting.
...
# 32. Final checklist
- [ ] Trình bày được cơ chế đúng chủ đề Linux Troubleshooting.
