# Scenario-based Questions - Exhaustive Interview Preparation Guide

## 1. Mục tiêu học
- Hiểu rõ về Scenario-based Questions trong môi trường Enterprise.
...
- **Enterprise Context:** Scenario-based Questions được áp dụng rộng rãi để mở rộng hệ thống.
- **Enterprise Context (Doanh nghiệp Enterprise):** Áp dụng Scenario-based Questions để đáp ứng JD 60% System + 40% DevOps, tối ưu chi phí và tăng tính ổn định (nhất là hệ thống TOS).
- **Candidate CV Alignment:** Mapping kinh nghiệm AWS, EKS, Docker, K8s, Terraform, ArgoCD vào bối cảnh Scenario-based Questions.

## 4. Kiến trúc / Cách hoạt động (ASCII Diagrams)
```text
[ Client ] --> [ Load Balancer ] --> [ Scenario-based Questions Component A ]
                                     |
                                     +--> [ Scenario-based Questions Component B ]
```
...
- Các tool đặc thù của Scenario-based Questions.
...
```yaml
# Sample config for Scenario-based Questions
server:
  port: 8080 
...
## 14. So sánh
| Feature | Scenario-based Questions | Alternative |
| --- | --- | --- |
...
- **Basic:** Scenario-based Questions là gì?
- **Intermediate:** Nêu cách backup/restore?
- **Advanced:** Làm sao để scale Scenario-based Questions cho 1 triệu RPS?
- **Production/Architecture:** Thiết kế Scenario-based Questions đa vùng (Multi-AZ).
...
- **Q:** Scenario-based Questions dùng để làm gì? -> **A:** Giải pháp cho ...
