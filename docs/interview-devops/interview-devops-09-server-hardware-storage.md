# Server Hardware and Storage — Cẩm nang phỏng vấn

# 1. Mục tiêu học
CPU/RAM/RAID/HBA, SAN/NAS, IOPS, latency, firmware và predictive failure; liên hệ concept với vận hành, failure mode và quyết định production.

# 2. Kiến thức nền cần có
Linux, networking, storage, identity, scripting, observability và change management.

# 3. Tổng quan kiến trúc
Mô tả control plane, data plane, state, dependency, traffic flow và failure domain của Server Hardware and Storage.

# 4. Cách hoạt động
Mô tả lifecycle từ request/config đến execution, persistence, response, audit và metric.

# 5. Thành phần và failure mode
Server Hardware and Storage có thể gặp resource exhaustion, network partition, stale state, permission error, disk failure hoặc bad change; xác định impact của từng lỗi.
...
ipmitool, smartctl, storcli, multipath, iostat
...
# 16. Knowledge check
Giải thích flow, failure domain, metric quan trọng, cách khoanh vùng và tiêu chí rollback của Server Hardware and Storage.
...
# 32. Final checklist
- [ ] Trình bày được cơ chế đúng chủ đề Server Hardware and Storage.
