# CV-based Questions - Exhaustive Interview Preparation Guide

## 1. Mục tiêu học
- Hiểu rõ về CV-based Questions trong môi trường Enterprise.
...
- **Enterprise Context:** CV-based Questions được áp dụng rộng rãi để mở rộng hệ thống.
- **Enterprise Context (Doanh nghiệp Enterprise):** Áp dụng CV-based Questions để đáp ứng JD 60% System + 40% DevOps, tối ưu chi phí và tăng tính ổn định (nhất là hệ thống TOS).
- **Candidate CV Alignment:** Mapping kinh nghiệm AWS, EKS, Docker, K8s, Terraform, ArgoCD vào bối cảnh CV-based Questions.

## 4. Kiến trúc / Cách hoạt động (ASCII Diagrams)
```text
[ Client ] --> [ Load Balancer ] --> [ CV-based Questions Component A ]
                                     |
                                     +--> [ CV-based Questions Component B ]
```
...
- Các tool đặc thù của CV-based Questions.
...
```yaml
# Sample config for CV-based Questions
server:
  port: 8080 
...
## 14. So sánh
| Feature | CV-based Questions | Alternative |
| --- | --- | --- |
...
- **Basic:** CV-based Questions là gì?
- **Intermediate:** Nêu cách backup/restore?
- **Advanced:** Làm sao để scale CV-based Questions cho 1 triệu RPS?
- **Production/Architecture:** Thiết kế CV-based Questions đa vùng (Multi-AZ).
...
- **Q:** CV-based Questions dùng để làm gì? -> **A:** Giải pháp cho ...
