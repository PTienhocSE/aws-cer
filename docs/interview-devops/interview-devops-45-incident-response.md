# Incident Response - Exhaustive Interview Preparation Guide

## 1. Mục tiêu học
- Hiểu rõ về Incident Response trong môi trường Enterprise.
...
- **Enterprise Context:** Incident Response được áp dụng rộng rãi để mở rộng hệ thống.
- **Enterprise Context (Doanh nghiệp Enterprise):** Áp dụng Incident Response để đáp ứng JD 60% System + 40% DevOps, tối ưu chi phí và tăng tính ổn định (nhất là hệ thống TOS).
- **Candidate CV Alignment:** Mapping kinh nghiệm AWS, EKS, Docker, K8s, Terraform, ArgoCD vào bối cảnh Incident Response.

## 4. Kiến trúc / Cách hoạt động (ASCII Diagrams)
```text
[ Client ] --> [ Load Balancer ] --> [ Incident Response Component A ]
                                     |
                                     +--> [ Incident Response Component B ]
```
...
- Các tool đặc thù của Incident Response.
...
```yaml
# Sample config for Incident Response
server:
  port: 8080 
...
## 14. So sánh
| Feature | Incident Response | Alternative |
| --- | --- | --- |
...
- **Basic:** Incident Response là gì?
- **Intermediate:** Nêu cách backup/restore?
- **Advanced:** Làm sao để scale Incident Response cho 1 triệu RPS?
- **Production/Architecture:** Thiết kế Incident Response đa vùng (Multi-AZ).
...
- **Q:** Incident Response dùng để làm gì? -> **A:** Giải pháp cho ...
