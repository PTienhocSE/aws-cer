# PostgreSQL — Cẩm nang phỏng vấn

# 1. Mục tiêu học
PostgreSQL dùng MVCC để reader không chặn writer, WAL để durability/recovery, catalog để mô tả object và planner để chọn execution plan. Nắm được cách thiết kế, vận hành, quan sát và xử lý sự cố.
...
psql, pg_stat_activity, pg_locks, EXPLAIN (ANALYZE, BUFFERS), pg_stat_replication, pg_dump, pg_restore, pg_isready.
...
# 32. Final checklist
- [ ] Trình bày được cơ chế đúng chủ đề PostgreSQL.
