# Apache Kafka & Amazon MSK - Exhaustive Interview Preparation Guide

## 1. Mục tiêu học
- Hiểu rõ về Apache Kafka & Amazon MSK trong môi trường Enterprise.
...
- **Enterprise Context:** Apache Kafka & Amazon MSK được áp dụng rộng rãi để mở rộng hệ thống.
- **Enterprise Context (Doanh nghiệp Enterprise):** Áp dụng Apache Kafka & Amazon MSK để đáp ứng JD 60% System + 40% DevOps, tối ưu chi phí và tăng tính ổn định (nhất là hệ thống TOS).
- **Candidate CV Alignment:** Mapping kinh nghiệm AWS, EKS, Docker, K8s, Terraform, ArgoCD vào bối cảnh Apache Kafka & Amazon MSK.

## 4. Kiến trúc / Cách hoạt động (ASCII Diagrams)
```text
[ Client ] --> [ Load Balancer ] --> [ Apache Kafka & Amazon MSK Component A ]
                                     |
                                     +--> [ Apache Kafka & Amazon MSK Component B ]
```
...
- Các tool đặc thù của Apache Kafka & Amazon MSK.
...
```yaml
# Sample config for Apache Kafka & Amazon MSK
server:
  port: 8080 
...
## 14. So sánh
| Feature | Apache Kafka & Amazon MSK | Alternative |
| --- | --- | --- |
...
- **Basic:** Apache Kafka & Amazon MSK là gì?
- **Intermediate:** Nêu cách backup/restore?
- **Advanced:** Làm sao để scale Apache Kafka & Amazon MSK cho 1 triệu RPS?
- **Production/Architecture:** Thiết kế Apache Kafka & Amazon MSK đa vùng (Multi-AZ).
...
- **Q:** Apache Kafka & Amazon MSK dùng để làm gì? -> **A:** Giải pháp cho ...
