# Kafka / Amazon MSK — Cẩm nang phỏng vấn

# 1. Mục tiêu học
Kafka phân tán log theo partition; broker lưu segment, producer ghi theo key, consumer group chia assignment và commit offset. MSK quản lý broker lifecycle nhưng application vẫn chịu trách nhiệm capacity, ACL, schema và lag. Nắm được cách thiết kế, vận hành, quan sát và xử lý sự cố.
...
kafka-topics, kafka-consumer-groups, kafka-configs, kafka-acls, aws kafka describe-cluster.
...
# 32. Final checklist
- [ ] Trình bày được cơ chế đúng chủ đề Kafka / Amazon MSK.
