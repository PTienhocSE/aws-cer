# Interview DevOps - Kubernetes Networking

## 1. Mục tiêu học 🔴
Nắm vững kiến thức nền tảng và nâng cao về Kubernetes Networking, hiểu rõ cách công nghệ này vận hành trong môi trường Enterprise, đặc biệt tập trung vào bối cảnh hệ thống Logistics và quản lý hệ thống Enterprise tại Enterprise System. Định hình khả năng Troubleshooting và thiết kế giải pháp High Availability.
...
## 9. Log 🔴
- **Vị trí**: System logs thường nằm ở `/var/log/` hoặc xem qua `journalctl -u kubernetes networking`. Application logs được stream ra `stdout/stderr`.
...
## 11. Configuration 🔴
```yaml
# Mẫu cấu hình tiêu chuẩn cho Kubernetes Networking trong môi trường Prod
apiVersion: v1
kind: Configuration
metadata:
  name: Kubernetes Networking-prod-config
spec:
  replicas: 3
  resources:
    requests:
...
## 14. So sánh 🟠
- So sánh Kubernetes Networking với các công nghệ tương đương trên thị trường (Ví dụ: K8s vs Docker Swarm, GitLab CI vs GitHub Actions).
...
1. [Cơ bản] Kubernetes Networking là gì và giải quyết bài toán nào?
2. [Cơ bản] Các thành phần chính của kiến trúc?
3. [Bản chất] Làm sao Kubernetes Networking đảm bảo tính HA?
4. [Bản chất] Mô tả lifecycle của một request đi qua Kubernetes Networking?
5. [Troubleshooting] Khi node bị down, Kubernetes Networking xử lý như thế nào?
*(Tổng cộng 30 câu hỏi: 10 cơ bản, 10 hiểu bản chất, 10 troubleshooting)*

## 17. Câu hỏi phỏng vấn 🔴
- Hãy kể một lần bạn gặp sự cố production lớn nhất với Kubernetes Networking và cách bạn giải quyết?
- Làm sao để thiết kế Kubernetes Networking cho hệ thống có hàng triệu request mỗi ngày?
...
- **Q**: Port mặc định của Kubernetes Networking là gì? -> **A**: ...
- **Q**: Lệnh xem log của Kubernetes Networking? -> **A**: ...
